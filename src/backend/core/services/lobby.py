...
class LobbyService:
    ...
    def can_bypass_lobby(room, user, role) -> bool:
        ...
        The rule itself is Room.is_joinable_by, which the room API applies before
        handing out a LiveKit token. Both have to answer the same way, or someone
        is offered a pass to a room the lobby would have held them out of.
